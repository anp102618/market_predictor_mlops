...
class ModelTuner:
    def __init__(self, config_path, allowed_models=None):
        ...
        self.df = pd.read_csv("Data/processed_data/preprocessed_data.csv", index_col=[0])
...
if __name__ == "__main__":
    try:
        config_path = "Config_Yaml/regressors.yaml"
        allowed_models = ["lasso", "ridge", "xgboost", "lstm"] 

        tuner = ModelTuner(config_path=config_path, allowed_models=allowed_models)
        tuner.run_grid_search()
        tuner.save_results("Config_Yaml/tuned_model.yaml")
